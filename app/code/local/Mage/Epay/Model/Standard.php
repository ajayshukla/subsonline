...
class Mage_Epay_Model_Standard extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getOrderPlaceRedirectUrl()
    {
    	...
    	if ($this->getConfigData('integrated', $this->getOrder() ? $this->getOrder()->getStoreId() : null) == '1') {
    		//return "https://relay.ditonlinebetalingssystem.dk/relay/v2/relay.cgi/" . Mage::getUrl('epay/standard/redirect') . "?forcerelay=1&HTTP_COOKIE=" . urlencode(getenv("HTTP_COOKIE"));// ";// . "?forcerelay=1&HTTP_COOKIE=" . getenv("HTTP_COOKIE");
    		return Mage::getUrl('epay/standard/redirect') . "?redirect=1";
		} else {
    		return Mage::getUrl('epay/standard/redirect');
    	} 
    }
	...
	public function calcCardtype($cardid)
	{
		switch($cardid)
		{
			...
			case 2:
				return 'VISA DANKORT';
			case 3:
				return 'VISA ELECTRON FOREIGN';
			case 4:
				return 'MASTERCARD';
			case 5:
				return 'MASTERCARD FOREIGN';
			case 6:
			...
			case 12:
				return 'EDK';
			case 13:
				return 'DINERS FOREIGN';
			case 14:
				return 'AMERICAN EXPRESS FOREIGN';
			case 15:
				return 'MAESTRO FOREIGN';
			case 16:
			...
			case 22:
				return 'Danske Netbetaling';
			case 24:
				return 'LIC MASTERCARD';
			case 25:
				return 'LIC MASTERCARD FOREIGN';
		}
	}
	...
	//
    public function calcMd5Key($order)
    {
		$res = "";
		if (((int)$this->getConfigData('md5type', $order ? $order->getStoreId() : null)) == 2)
		{
			$res = md5($this->convertToEpayCurrency($order->getBaseCurrency()->getCode()) . ($order->getBaseTotalDue() * 100) . $order->getRealOrderId() . $this->getConfigData('md5key', $order ? $order->getStoreId() : null));
		}
		return $res;
    }
    ...
    //
    // Simply return the url for the ePay Standard Payment window
    //
    public function getEpayUrl()
    {
    	if ($this->getConfigData('integrated', $this->getOrder() ? $this->getOrder()->getStoreId() : null) == '1')
		{
			return "https://ssl.ditonlinebetalingssystem.dk/auth/default.aspx";
		}
		else
		{
			return "https://ssl.ditonlinebetalingssystem.dk/popup/default.asp";
		}
    }
	...
	{
		$logoArray = explode(',', $this->getConfigData('epaylogos', $this->getOrder() ? $this->getOrder()->getStoreId() : null));
		foreach ($logoArray as $item)
		{
			if($item == '1000' || $item == '1002' || $item == '1003' || $item == '1004')
			{
				return true;
			}
		}
    ...
    public function showCardsList()
	{
		...
		foreach ($logoArray as $item)
		{
			if($item == '1' || $item == '2' || $item == '3' || $item == '4' || $item == '5' || $item == '6' || $item == '7' || $item == '8' || $item == '9' || $item == '10' || $item == '11' || $item == '12' || $item == '13')
			{
				return true;
			}
		}
