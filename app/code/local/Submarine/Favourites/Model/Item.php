...
/**
 * Favourites item model
 *
 * @method Submarine_Favourites_Model_Resource_Item _getResource()
 * @method Submarine_Favourites_Model_Resource_Item getResource()
 * @method int getFavouritesId()
 * @method Submarine_Favourites_Model_Item setFavouritesId(int $value)
 * @method int getProductId()
 * @method Submarine_Favourites_Model_Item setProductId(int $value)
 * @method int getStoreId()
 * @method Submarine_Favourites_Model_Item setStoreId(int $value)
 * @method string getAddedAt()
 * @method Submarine_Favourites_Model_Item setAddedAt(string $value)
 * @method string getDescription()
 * @method Submarine_Favourites_Model_Item setDescription(string $value)
 *
 * @category    Mage
 * @package     Submarine_Favourites
 * @author      Magento Core Team <core@magentocommerce.com>
 */
class Submarine_Favourites_Model_Item extends Mage_Core_Model_Abstract
    implements Mage_Catalog_Model_Product_Configuration_Item_Interface
{
     ...
     * @var string
     */
    protected $_customOptionDownloadUrl = 'favourites/index/downloadCustomOption';
   ...
   /**
     ...
     * @var string
     */
    protected $_eventPrefix = 'favourites_item';
    ...
    {
        $this->_init('favourites/item');
    }

    /**
     * Retrieve resource instance wrapper
     *
     * @return Submarine_Favourites_Model_Mysql4_Item
     */
     ...
     *
     * @param   Submarine_Favourites_Model_Item_Option $option
    ...
    {
        if (!$this->getFavouritesId()) {
            Mage::throwException(Mage::helper('favourites')->__('Cannot specify favourites.'));
        }
        if (!$this->getProductId()) {
            Mage::throwException(Mage::helper('favourites')->__('Cannot specify product.'));
        }
    ...
    /**
     * Retrieve favourites item data as array
     *
    ...
    public function getDataForSave()
    {
        ...
        $data['favourites_id'] = $this->getFavouritesId();
    ...
    /**
     * Load item by product, favourites and shared stores
     *
     ...
     * @return Submarine_Favourites_Model_Item
     */
    public function loadByProductFavourites($favouritesId, $productId, $sharedStores)
    {
        $this->_getResource()->loadByProductFavourites($this, $favouritesId, $productId, $sharedStores);
        $this->_afterLoad();
        $this->setOrigData();
    ...
    {
        if (is_array($option)) {
            $option = Mage::getModel('favourites/item_option')->setData($option)
                ->setItem($this);
        } else if ($option instanceof Submarine_Favourites_Model_Item_Option) {
            $option->setItem($this);
        } else if ($option instanceof Varien_Object) {
            $option = Mage::getModel('favourites/item_option')->setData($option->getData())
     ...
     *
     * @param   string $code
     * @return  Submarine_Favourites_Model_Item_Option || null
     */
    ...
    public function loadWithOptions($id, $optionsFilter = null)
    {
        ...
        $options = Mage::getResourceModel('favourites/item_option_collection')
            ->addItemFilter($this);
        if ($optionsFilter) {
